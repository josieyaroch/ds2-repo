...
Maybe there is a way to make a different plot for each season and just look at episodes for each of the seasons - Jake G
...
making a separate plot for each season as comment above suggests-
...
df <- theoffice |>
  ...
  group_by(season, episode, season_episode) |> 
  summarize(
    pos_sent = sum(sentiment == "positive"),
    neg_sent = sum(sentiment == "negative"),
    sent_score = pos_sent - neg_sent,
    .groups = "drop"
  ) |> 
...
need to change x-axis to be just episode, not season_episode. As is, you can't really see the trends and have a lot of empty space on each plot
...
I like how each of the seasons are represented by colors, but I see that S01 and S02 are together on the x-axis - Jake G
...
Very cool Anova table, but in the graph I think that hollow points don't help the figure visually - Jake G

I'm confused on what is meant by hollow points in the above comment.
...
Maybe use lexicon list of negative words to help with negation - Jake G

```{r}
neg_bing <- get_sentiments("bing") |> 
...
theoffice0 |> 
  ...
  inner_join(sentiments, by = c("word2" = "word")) |>   # Merge with Sentiment Lexicon 
  count(word1, sort = T) |>  
  filter(word1 %in% neg_bing)  |> 
  group_by(word1) |> 
  count(word1, sort = T)


#in bigram, if word1 %in% neg_bing & word2 is a sentiment (already filtered for this), then reverse the sentiment of word2
theoffice0 |> 
  # filter(name == "Romeo and Juliet") |>  # only RandJ 
  select(text) |> 
  unnest_tokens(bigram, text, "ngrams", n = 2) |> # Tokenize
...
Using the bing negative words lexicon didn't seem to affect the resulting plot, but is definitely easier than listing out negation words and ensures no words get left out.
